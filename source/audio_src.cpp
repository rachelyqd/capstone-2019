...
void MainWindow::onConnectAudioSrc()
{
    ...
    wiced_bt_audio_source_connect_data_t data;
...
// Disconnect from first peer device
void MainWindow::onDisconnectAudioSrc()
{
    ...
    if (NULL == pDev)
        //Log("can't read device \n");
        return;

    Log("disconnecting...");
    UINT8* address = pDev->m_address;
    Log("disconnecting %02X:%02X:%02X:%02X:%02X:%02X \n", address[0], address[1], address[2], address[3], address[4], address[5]) ;
    Log("handle %d \n",pDev->m_audio_handle);
...
{
    uint8_t sample_freq = (ui->cbSineFreq->currentIndex() == 0) ? AUDIO_SRC_SAMPLING_FREQ_44_1 : AUDIO_SRC_SAMPLING_FREQ_48;
    uint8_t audio_mode = ui->rbAudioModeMono->isChecked() ? AUDIO_SRC_CHANNEL_MODE_MONO : AUDIO_SRC_CHANNEL_MODE_STEREO;

    if (!m_audio_started
            && ui->rbAudioSrcFile->isChecked()
            && !InitializeAudioFile())
        return;
    ...
    /*pDev = GetConnectedAudioSrcDevice2();
    if (pDev != NULL)
        app_host_audio_src_start(pDev->m_address, sample_freq, audio_mode);*/
}
...
void MainWindow::on_btnHelpAVSRC_clicked()
{
    ...
    Log("");
    Log("WICED Platforms : 20706-A2, 20719-B1");
    Log("Apps : demo.watch");
    Log("");
