...
#if ( defined(CYW20719B1) || defined(CYW43012C0) || defined(CYW20721B1) )
...
{
/* samp_freq    */    (A2D_SBC_IE_SAMP_FREQ_44|A2D_SBC_IE_SAMP_FREQ_48),
...
uint8_t hci_control_audio_handle_command( uint16_t cmd_opcode, uint8_t* p_data, uint32_t len )
{
    ...
    case HCI_CONTROL_AUDIO_STOP:
        ...
        status = a2dp_app_hci_control_stop(p_data, len) == WICED_SUCCESS ?
                HCI_CONTROL_STATUS_SUCCESS : HCI_CONTROL_STATUS_FAILED;
        break;
#if ( defined(CYW20719B1) || defined(CYW20721) )
    case HCI_CONTROL_AUDIO_READ_STATISTICS:
        {
            ...
            {
                WICED_BT_TRACE ("\n Delay [%d - %d ]msec ==> Pkts : %d",index*10,((index+1)*10)-1,a2dp_stat.packet_ack_stats_table[index]);
            }
            WICED_BT_TRACE ("\n Delay > 100msec ==> Pkts : %d",a2dp_stat.packet_ack_stats_table[index]);
    ...
    if ( ( a2d_status == A2D_SUCCESS ) && ( peer_codec_info[2] == A2D_MEDIA_CT_SBC ) )
    {
        if (!av_app_cb.is_accepter)
        {
            ...
            sbc_caps->samp_freq = ( sample_freq == AUDIO_SF_16K )   ? A2D_SBC_IE_SAMP_FREQ_16 :
                                  ...
                                  ( sample_freq == AUDIO_SF_44_1K ) ? A2D_SBC_IE_SAMP_FREQ_44 :
                                  ( sample_freq == AUDIO_SF_48K )   ? A2D_SBC_IE_SAMP_FREQ_48 :
...
static void av_app_getcap_cmpl( uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data )
{
    ...
    if (av_app_cb.state == AV_STATE_CONNECTED)
    {
        ...
        wiced_bt_app_start_conn_idle_timer( 2, av_app_send_setconfiguration );
    }
}
 ...
 */
static void av_app_connect_event_hdlr(uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    WICED_BT_TRACE( "[%s]: \n\r", __FUNCTION__);
                                          ...
                                          wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    {
        case AVDT_CONNECT_IND_EVT:
            av_app_connect_event_hdlr( handle, bd_addr, event, p_data );
            break;
...
static wiced_result_t av_app_send_discover_req( void )
{
    ...
    if (p_sep_info != NULL)
    {
        ...
        if ( AVDT_SUCCESS != wiced_bt_avdt_discover_req( av_app_cb.peer_bda, p_sep_info, AV_NUM_SEPS, av_app_disc_results ) )
        {
            WICED_BT_TRACE( "[%s] failed!!\n\r", __FUNCTION__ );
            wiced_bt_free_buffer(p_sep_info);
            status = WICED_ERROR;
 ...
 */
void av_app_sdp_cback( wiced_bool_t found, wiced_bt_a2d_service_t *p_service )
{
    ...
    if ( found )
    {
        ...
        av_app_cb.peer_avdt_version = p_service->avdt_version;
    ...
    if (av_app_cb.is_accepter)
    {
        av_app_connect_event_hdlr( av_app_cb.avdt_handle, av_app_cb.peer_bda, AVDT_CONNECT_IND_EVT, NULL);
        return;
    }
...
wiced_result_t av_app_initiate_sdp( BD_ADDR bda )
{
    wiced_bt_a2d_sdp_db_params_t db_params;
    ...
    /* set up parameters */
    db_params.db_len   = SDP_DB_LEN;
    db_params.num_attr = 3;
    db_params.p_db     = av_app_cb.p_sdp_db;
    db_params.p_attrs  = attr_list;

    /* Search the contents of the database for the A2DP Sink service */
    if ( A2D_FindService( UUID_SERVCLASS_AUDIO_SINK, bda, &db_params, av_app_sdp_cback ) == A2D_SUCCESS )
    {
...
void av_app_init( void )
{
    ...
    if ( av_app_memInit( ) )
    {
        avdt_init( );
#if ( WICED_APP_AUDIO_RC_TG_INCLUDED == TRUE )
        hci_control_rc_target_init( );    /* Do AVCT init */
#endif
