...
/**
 * @anchor OTA_FW_UPGRADE_STATUS
 * @name OTA firmware upgrade status codes.
 * @{
 */
#define OTA_FW_UPGRADE_STATUS_STARTED                           1   /**< Client started OTA firmware upgrade process */
#define OTA_FW_UPGRADE_STATUS_ABORTED                           2   /**< OTA firmware upgrade process has been aborted or failed verification */
#define OTA_FW_UPGRADE_STATUS_COMPLETED                         3   /**< OTA firmware upgrade completed, will reboot */
/** @} OTA_FW_UPGRADE_STATUS */
...
/**
 * \brief Optional callback to be executed when OTA Fimware Upgrade state changes
 * \ingroup wiced_firmware_upgrade
 *
 * \details Application can register the callback to be executed when client starts,
 * aborts or completes OTA firmware upgrade process.  In the case when the firmware
 * has been downloaded and verified and before the callback will be executed before
 * the library executes the restart.  This can be useful for example if application
 * needs to drop all connections.
 *
 * \param   status : OTA firwmare upgrade status (see @ref OTA_FW_UPGRADE_STATUS "OTA firmware upgrade status codes")
 */
 ...
 * \ingroup wiced_firmware_upgrade
 *
 * \details This function is typically called by the application during initialization. The none-zero pointer to the
 * public key indicates to the library that the secure OTA firmware update procedure is being used.
 *
...
/**
 * \brief Check the state of the OTA Firmware Upgrade
 * \ingroup wiced_firmware_upgrade
 *
 * \details This function returns TRUE if OTA Firmware Upgrade is active.
 */
