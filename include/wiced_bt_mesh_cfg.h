...
typedef struct
{
    ...
    uint8_t       *trigger_delta_down;                                       /**< Delta down value that triggers a status message */
    uint8_t       *trigger_delta_up;                                         /**< Delta up value that triggers a status message */
...
typedef PACKED struct
{
    ...
    uint32_t    poll_timeout;           ///< Poll timeout in 100ms unite to bt requested by the Low Power node. It should be between 0x00000a and 0x34bbff
...
/**
 * Defines configuration data for wiced_bt_mesh_core_init()
 */
typedef PACKED struct
{
    ...
    uint16_t                                replay_cache_size;  ///< Value representing the minimum number of replay protection list entries in a device 
