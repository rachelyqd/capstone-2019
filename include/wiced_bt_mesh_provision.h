...
#define WICED_BT_MESH_UNPROVISIONED_DEVICE                  201   ///< Unprovisioned device info
#define WICED_BT_MESH_PROVISION_LINK_STATUS                 202   ///< Provision Linke established or dropped
...
typedef struct
{
    ...
    uint8_t  name[32];                  ///< device name if present in the scan response data
...
/// This structure contains information to start or stop unprovisioned scan
typedef struct
{
    uint8_t  start;                     ///< Set to TRUE to start scan
} wiced_bt_mesh_provision_scan_unprovisioned_data_t;
...
{
    uint32_t conn_id;                   ///< GATT connection ID, or any number if advertising channel should be used.
    uint16_t addr;                      ///< address of the first element of the device if assigned
...
{
    uint32_t conn_id;                   ///< GATT connection ID, or any number if advertising channel should be used.
    uint32_t addr;                      ///< Address to assign to the device during provisioning
...
{
    uint8_t  ttl;                       ///< New Default TTL value
    uint8_t  received_ttl;              ///< Value of the TTL that this message has been received
                                        ///< Client can calculate number of hops to the SRC compairing this and default TTL values.
...
#define OPERATION_UPDATE     2 ///< Update network key
 ...
 * @return   WICED_TRUE/WICED_FALSE - success/failed.
 */
wiced_bool_t wiced_bt_mesh_provision_connect(wiced_bt_mesh_provision_connect_data_t *p_data, uint8_t use_gatt);
...
/*
 * \brief GATT Connection State
 * \details When running on host and the application controls connection using OS services, the application should 
 * notify when a connection has been established or dropped.  Connection up implies that that the GATT connection, 
 ...
 */
void wiced_bt_mesh_gatt_client_connection_state_changed(uint16_t conn_id);
 ...
 *   data_in_handle:     Handle for sending data in
 *   data_out_handle:    Handle for receiving data 
 ...
 *           WICED_FALSE - failed
 */
wiced_bool_t wiced_bt_mesh_provision_gatt_connected(uint32_t conn_id, uint16_t data_in_handle, uint16_t data_out_handle, uint8_t *uuid, uint8_t identify_duration);
 ...
 *
 * @param[in]  p_data Pointer to the data structure with the address of the peer device and new GATT Proxy state.
 *
 * @return   WICED_TRUE/WICED_FALSE - success/failed.
 */
 ...
 *
 * @param[in]  p_set Pointer to the data structure with the address of the element and the new period.
 *
 * @return   WICED_TRUE/WICED_FALSE - success/failed.
 */
 ...
 *
 * @param[in]  p_get Pointer to the data structure with the address of the LPN.
 *
 * @return   WICED_TRUE/WICED_FALSE - success/failed.
 */
