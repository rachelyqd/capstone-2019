...
/** To register the callback to receive the audio data
 *
 *  Called by the application to receive the audio data. Route should be set to AUDIO_ROUTE_APP when using this.
 *  Receives raw PCM samples in case of SBC codec and encoded audio data in case of AAC codec
 *
 *  @param p_cback    callback to be invoked with the received audio data
 *
 *  @return none
 */
void wiced_audio_sink_register_data_cback( void* p_cback );
