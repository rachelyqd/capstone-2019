...
/**
 * Setup accessory parameters in application library
 */
void homekit_app_accessory_setup(wiced_btle_hap_info_t* hap_info, homekit_event_cback_t* event_cback);

/**
 * This function is usually called in HOMEKIT_EVENT_SET_CHARACTERISTIC_VALUE callback.  However application
 * can return from event callback first and call this function later when set value operation is completed.
...
/**
 * This function is usually called in HOMEKIT_EVENT_GET_CHARACTERISTIC_VALUE callback.  However application
 * can return from event callback first and call this function later when get value operation is completed.
 */
