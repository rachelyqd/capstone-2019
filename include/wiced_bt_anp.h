...
/** @file
 *
 * This file provides common definitions of the Alert Notification Server(ANS) and Alert Notification Client(ANC)
 */
...
#define ANP_ALERT_CATEGORY_ID_ALL_CONFIGURED        0xff /**< Special value used by only Alert notification client in notify immedietely control 
                                                                                                point GATT write operation. This enables the client to receive missing alerts qucikly during connection loss recovery */
typedef uint8_t wiced_bt_anp_alert_category_id_t;
/** @} ANP_ALERT_CATEGORY_ID */

/**
 * @anchor ANP_ALERT_CATEGORY_ENABLE
 * @name Alert category enable bit mask
 * @{ */
