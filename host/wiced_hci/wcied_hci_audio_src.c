...
bool wiced_hci_audio_src_connect(wiced_bt_audio_source_connect_data_t *p_data)
{
    ...
    return  wiced_hci_send_command(HCI_CONTROL_AUDIO_COMMAND_CONNECT, cmd, p_cmd - cmd);
}

bool wiced_hci_audio_src_disconnect(wiced_bt_ag_disconnect_data_t *p_data)
{
    uint8_t    cmd[10];
    uint8_t     *p_cmd = cmd;
    ...
    return wiced_hci_send_command(HCI_CONTROL_AUDIO_COMMAND_DISCONNECT, cmd, p_cmd - cmd);
...
bool wiced_hci_audio_src_audio_start(wiced_bt_audio_source_start_data_t *p_data)
{
    ...
    return wiced_hci_send_command(HCI_CONTROL_AUDIO_START, cmd, p_cmd - cmd);
...
bool wiced_hci_audio_src_audio_stop(wiced_bt_audio_source_stop_data_t *p_data)
{
    ...
    return wiced_hci_send_command(HCI_CONTROL_AUDIO_STOP, cmd, p_cmd - cmd);
}

bool wiced_hci_audio_src_audio_data(wiced_bt_audio_source_audio_data_t *audio_data)
{
