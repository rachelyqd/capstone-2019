...
{
    wiced_bt_ag_connect_connect_data_t data;
...
{
    wiced_bt_ag_disconnect_data_t data;
    wiced_bt_device_t* p_dev = app_host_find_device(bda);    
    if(p_dev && (p_dev->m_ag_handle != WICED_NULL_HANDLE))
    {
        ...
        p_dev->m_app_state.ag_state.ag_audio_opened = false;
...
{
    wiced_bt_ag_audio_open_data_t data;
    wiced_bt_device_t* p_dev = app_host_find_device(bda);
    if(p_dev && (p_dev->m_ag_handle != WICED_NULL_HANDLE))
    {
...
{
    wiced_bt_ag_audio_close_data_t data;
    wiced_bt_device_t* p_dev = app_host_find_device(bda);
    if(p_dev && (p_dev->m_ag_handle != WICED_NULL_HANDLE))
    {
    ...
    switch (opcode)
    {
        ...
        case HCI_CONTROL_AG_EVENT_OPEN:
        {
            ...
            if (p_data[8] == HCI_CONTROL_HF_STATUS_SUCCESS)
            {
                ...
                device->m_app_state.ag_state.ag_audio_opened = false;
        ...
        {
            handle = p_data[0] | (p_data[1] << 8);
            device = app_host_find_device_by_connection(WICED_CONNECTION_TYPE_AG);
            if(device)
            {
        ...
        case HCI_CONTROL_AG_EVENT_AUDIO_OPEN:
        {
            ...
            if(device)
            {
                device->m_app_state.ag_state.ag_audio_opened = true;
            }
