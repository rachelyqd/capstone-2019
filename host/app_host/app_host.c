...
{
    bool found = false;
    wiced_bt_device_t *p_dev_list = g_app.p_devices;
    wiced_bt_device_t *p_dev_previous = 0;
    while(p_dev_list)
    {
        ...
        p_dev_list = (wiced_bt_device_t *)p_dev_list->p_next;

    }

    if(!found)
    {
        p_dev_list = (wiced_bt_device_t *)malloc(sizeof(wiced_bt_device_t));
        memset(p_dev_list, 0, sizeof(wiced_bt_device_t));
        memcpy((p_dev_list->m_address), bda, BDA_LEN);

        if(p_dev_previous)
            p_dev_previous->p_next = p_dev_list;
        else
             g_app.p_devices = p_dev_list;
    }
    ...
    if(found)
    {
        if(p_dev_previous)
            p_dev_previous->p_next = p_dev_list->p_next;
             ...
             g_app.p_devices = (wiced_bt_device_t *)p_dev_list->p_next;
...
{
    wiced_bt_device_t *p_dev_list =  g_app.p_devices;
    wiced_bt_device_t *p_dev_next = 0;
    while(p_dev_list)
    {
...
// Find a device in list of devices using connection type
