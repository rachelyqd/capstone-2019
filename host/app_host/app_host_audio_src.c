...
{
    wiced_bt_device_t* p_dev = app_host_find_device(p_data->bda);
...
{
    wiced_bt_audio_source_disconnect_data_t data;
    wiced_bt_device_t* p_dev = app_host_find_device(bda);
    app_host_log("%02X:%02X:%02X:%02X:%02X:%02X \n", bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
    if (!p_dev)
        app_host_log("not detecting anything");
    if(p_dev && (p_dev->m_audio_handle != WICED_NULL_HANDLE))
    {
...
{
    wiced_bt_audio_source_start_data_t start;
    wiced_bt_device_t* p_dev = app_host_find_device(bda);
...
{
    wiced_bt_audio_source_stop_data_t data;
    wiced_bt_device_t* p_dev = app_host_find_device(bda);
    if(!p_dev || (p_dev && (p_dev->m_audio_handle == WICED_NULL_HANDLE)))
    {
        app_host_log("AV not connected");
...
{
    wiced_bt_audio_source_audio_data_t audio_data;
    wiced_bt_device_t* p_dev = app_host_find_device_by_connection(WICED_CONNECTION_TYPE_AUDIO);
...
void app_host_audio_src_event(uint16_t opcode, uint8_t * p_data, uint16_t len)
{
    ...
    switch (opcode)
    {
        ...
        case HCI_CONTROL_AUDIO_EVENT_DISCONNECTED:
        {
            ...
            device = app_host_find_device_by_connection(WICED_CONNECTION_TYPE_AUDIO);
            if(device && (device->m_audio_handle == handle))
            {
        ...
        case HCI_CONTROL_AUDIO_EVENT_STARTED:
        {
            device = app_host_find_device_by_connection(WICED_CONNECTION_TYPE_AUDIO);
            if(device)
                device->m_app_state.audio_src_state.audio_started = true;
        ...
        case HCI_CONTROL_AUDIO_EVENT_STOPPED:
        {
            ...
            device = app_host_find_device_by_connection(WICED_CONNECTION_TYPE_AUDIO);
            if(device)
                device->m_app_state.audio_src_state.audio_started = false;
        }
        break;
