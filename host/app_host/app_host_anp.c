    ...
    case HCI_CONTROL_ANS_EVENT_ANS_ENABLED:
        {
            if(len == 2)
            {
                g_app.m_ans_supported_new_alerts = p_data[0] | (p_data[1] << 8);
                g_app.m_ans_supported_unread_alerts = p_data[0] | (p_data[1] << 8);
                g_app.m_ans_supported_alerts = p_data[0] | (p_data[1] << 8);
...
void app_host_anc_event(uint16_t opcode, uint8_t * p_data, uint16_t len)
{
    ...
    case HCI_CONTROL_ANC_EVENT_SERVER_SUPPORTED_NEW_ALERTS:
        status = p_data[0];
        g_app.m_anc_server_supported_new_alerts = p_data[1] | (p_data[2] << 8);
    ...
    case HCI_CONTROL_ANC_EVENT_SERVER_SUPPORTED_UNREAD_ALERTS:
        status = p_data[0];
        g_app.m_anc_server_supported_unread_alerts = p_data[1] | (p_data[2] << 8);
