...
void ancs_client_process_notification_source(uint8_t *data, int len)
{
    ...
    {
#ifdef CYW43012C0
        if ((p_ancs_event = (ancs_event_t *) watch_get_buffer(sizeof(ancs_event_t))) == NULL)
#else
...
void ancs_client_process_event_attribute(uint8_t  *data, int len)
{
    ...
    else
    {
        ...
        {
#ifdef CYW43012C0
            if ((ancs_client.p_first_event = (ancs_event_t *) watch_get_buffer(sizeof(ancs_event_t))) == NULL)
#else
...
//        attr_id  = data[5];
        ...
        if (attr_len <= len - 8)
        {
            ancs_client_process_event_attribute((char *)&data[5], len - 5);
        }
        else
        {
