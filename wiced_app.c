...
#endif
#ifdef CYW43012C0
    watch_app_pool_big = wiced_bt_create_pool(512, 2);
    watch_app_pool_small = wiced_bt_create_pool(128, 2);
#endif
#ifndef DEBUG
...
#ifdef CYW43012C0
void* watch_get_buffer(uint32_t size)
{
    WICED_BT_TRACE("size %d\n", size);

    if(size < 50)
        return wiced_bt_get_buffer_from_pool( watch_app_pool_small );
    else
        return wiced_bt_get_buffer_from_pool( watch_app_pool_big );
}

void watch_free_buffer( void* buffer_ptr )
{
    wiced_bt_free_buffer(buffer_ptr);
}
#endif
